...
Endpoints:
  ...
  - PATCH /api/superbills/{superbill_id}     — update superbill fields
...
async def get_financial_reports(
    ...
    unresolved claim aging, avg days to payment by payer, and YTD summary.
    ...
    # --- 6) Local unresolved claim aging ---
    unresolved_query = f"""
        SELECT
        ...
        {owner_filter}
    """
    unresolved_row = await pool.fetchrow(unresolved_query, *base_params)
    total_claims = unresolved_row["total_claims"] if unresolved_row else 0
    stale_claims = unresolved_row["stale_claims"] if unresolved_row else 0
    unresolved_claim_rate = {
        "total_claims": total_claims,
        "stale_claims": stale_claims,
        "rate_percent": round((stale_claims / total_claims * 100) if total_claims > 0 else 0, 1),
    }
    ...
    return {
        ...
        "ar_aging": ar_aging,
        "payer_mix": payer_mix,
        "unresolved_claim_rate": unresolved_claim_rate,
